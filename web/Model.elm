...
isSelected prayer model =
    model.openPrayer |> Maybe.map (\p -> p.id == prayer.id) |> Maybe.withDefault False
...
emptyPrayer id =
    { id = id, name = "", text = "", favorite = False }


updateWith : Prayer -> Prayer -> Prayer
updateWith newPrayer oldPrayer =
    if newPrayer.id == oldPrayer.id then
        newPrayer
...
updatePrayer p model =
    ...
    in
    { model
        | prayers = replacePrayer p prayers
...
deletePrayer p model =
    let
        prayers =
            List.remove p model.prayers

        openPrayer =
            maybeFilter (\o -> o.id /= p.id) model.openPrayer
    in
    { model
        | prayers = prayers
