...
import Model exposing (Model, Prayer, WindowSize, addPrayer, prayerEncode, windowSizeDecoder)
...
convertWsToMsg v =
    let
        res =
            Decode.decodeValue incomingMsgDecoder v
    in
    case res of
        Err e ->
            Error <| "unknown ws message: " ++ Decode.errorToString e
...
resizeToMsg v =
    let
        res =
            Decode.decodeValue windowSizeDecoder v
    in
    case res of
        Err e ->
            Error <| "unknown window size: " ++ Decode.errorToString e
...
incomingMsgDecoder =
    Decode.oneOf [ wsClientIdDecoder, wsPrayersDecoder, wsClosedDecoder ]


wsClientIdDecoder : Decoder IncomingMsg
wsClientIdDecoder =
    Decode.field "id" Decode.string |> Decode.map WsClientId


wsPrayersDecoder : Decoder IncomingMsg
wsPrayersDecoder =
    Decode.list Model.prayerDecoder |> Decode.map WsPrayers
    ...
    Decode.string
        |> Decode.andThen
            (\s ->
                case s of
                    "closed" ->
                        Decode.succeed WsClosed

                    "tunnel-open" ->
                        Decode.succeed WsTunnelOpen

                    _ ->
                        Decode.fail <| "unknown message: " ++ s
            )
...
update message model =
    case message of
        ...
        PrayerEditMsg m ->
            ( PrayerEdit.update m model, Cmd.none )

        New ->
            case List.find (\p -> String.isEmpty p.name) model.prayers of
                Just p ->
                    ( { model | openPrayer = Just p }, focusEditName )

                Nothing ->
                    ( model, Random.generate NewRandomId uuidGenerator )

        NewRandomId id ->
            ( addPrayer id model, focusEditName )

        Load ->
            ( model, wsSend <| Encode.string "load" )

        Save ->
            if List.any (\p -> p.name == "") model.prayers then
                ( { model | errors = [ "Empty prayer name" ] }, Cmd.none )

            else
                ( model, wsSend <| Encode.list prayerEncode model.prayers )

        WsIncoming (WsClientId clientId) ->
            ( { model | clientId = Just clientId }, Cmd.none )

        WsIncoming WsClosed ->
            ( { model | clientId = Nothing, appConnected = False }, Cmd.none )

        WsIncoming WsTunnelOpen ->
            ( { model | appConnected = True }
            , if List.isEmpty model.prayers then
                wsSend <| Encode.string "load"
        ...
        (column
            ...
            , row
                ...
                , case model.openPrayer of
                    ...
                    Just p ->
                        Element.map PrayerEditMsg (PrayerEdit.view (model.windowSize.width - listWidth - (5 * space)) p)
                ]
            , footer
            ]
...
clientIdView code appConnected =
    if appConnected then
        Element.el [ Font.size 32 ] (Element.text "Phone\nconnected")
